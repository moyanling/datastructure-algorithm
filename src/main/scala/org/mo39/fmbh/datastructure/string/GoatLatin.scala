...
/**
  ...
  * }}}
  *
  * @see [[https://leetcode.com/problems/goat-latin/ GoatLatin]]
  * @author mo39.fmbh
  */
...
object GoatLatin extends Enumerable[GoatLatin] {
  case object Solution extends GoatLatin {
    override def toGoatLatin(S: String): String = {
      val vowel = Set('a', 'e', 'i', 'o', 'u')
      val f = (word: String, i: Int) => {
        val suffix = Array.fill(i)('a').mkString
        if (vowel(word(0).toLower)) s"${word}ma$suffix"
        else s"${word.substring(1)}${word(0)}ma$suffix"
      }
      val arr = S.split(' ')
      arr.indices.map(i => f(arr(i), i + 1)).mkString(" ")
    }
  }
}
