...
/**
  ...
  * }}}
  *
  * @see [[https://leetcode.com/problems/license-key-formatting/ LicenseKeyFormatting]]
  * @author mo39.fmbh
  */
@ProblemSource(LeetCode)
sealed trait LicenseKeyFormatting {
  def licenseKeyFormatting(S: String, K: Int): String
}

object LicenseKeyFormatting extends Enumerable[LicenseKeyFormatting] {
  case object Solution extends LicenseKeyFormatting {
    override def licenseKeyFormatting(S: String, K: Int): String =
      S.replace("-", "").reverse.grouped(K).mkString("-").reverse.toUpperCase
  }
}
